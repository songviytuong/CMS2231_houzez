...
abstract class ContentBase
{
	...
	function LoadFromData(&$data, $loadProperties = false)
	{
		...
		//$this->mName                       = $data["content_name"];
                #Start MLE
                global $hls, $default_lang_fallback, $mleblock, $mleblockfallback;
                if (isset($hls)) {
                    $this->mLangReady = array();
                    foreach ($hls as $hl => $mle) {
                        if (empty($default_lang_fallback) && empty($data["menu_text_{$mle['block']}"]))
                            continue;
                        $this->mLangReady[] = $hl;
                    }
                }
                
                $this->mName = ($mleblock !== '_en' && $data["content_name{$mleblock}"]) ? $data["content_name{$mleblock}"] : $data["content_name"];
                #End MLE
		...
		//$this->mMenuText                   = $data['menu_text'];
                #Start MLE
                $this->mMenuText = ($mleblock !== '_en' && $data["menu_text{$mleblock}"]) ? $data["menu_text{$mleblock}"] : $data["menu_text"];
                #End MLE
	...
	{   
		...
		$this->mModifiedDate = trim($db->DBTimeStamp(time()), "'");
                if($mleblock === '_en'){    
                    ...
                    $query = "UPDATE ".CMS_DB_PREFIX."content SET content_name$mleblock = ?, owner_id = ?, type = ?, template_id = ?, parent_id = ?, active = ?, default_content = ?, show_in_menu = ?, cachable = ?, secure = ?, page_url = ?, menu_text$mleblock = ?, content_alias = ?, metadata = ?, titleattribute = ?, accesskey = ?, tabindex = ?, modified_date = ?, item_order = ?, last_modified_by = ? WHERE content_id = ?";
                }
	...
	protected function display_single_element($one,$adding)
	{
		...
		case 'title':
			...
			return array('<label for="in_title">*'.lang('title').':</label>'.$help,
						 '<input type="text" id="in_title" size="50" name="title" required="required" value="'.cms_htmlentities($this->mName).'" />');
