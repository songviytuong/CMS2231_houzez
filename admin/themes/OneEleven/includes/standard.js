...
( function(global, $) {'use strict';
    ...
    OE.view = {
        ...
        setUIButtons : function() {
            ...
            $('input[type="submit"],input[type="button"], :button[data-ui-icon]').each(function() {
                var button = $(this),
                ...
                if (!button.hasClass('noautobtn') || !button.hasClass('no-ui-btn')) {
                    ...
                    } else if (button.is('[name*=add]')) {
                        icon = button.data('uiIcon') ||'ui-icon-circle-plus';
                    } else if (button.is('[name*=resettodefault]') || button.attr('id') === 'refresh') {
                        icon = button.data('uiIcon') ||'ui-icon-refresh';
                    }
                }

                if (button.is(':button') && !button.is('[name*=add]')) {
                    label = button.text();
                }
