...
final class ContentListBuilder
{
  ...
  {
      $config = \cms_config::get_instance();
      $dflt = 'expand,icon1,hier,page,alias,url,template,friendlyname,owner,active,default,importseo,move,view,copy,edit,delete,multiselect';
      $mod = $this->_module;
      ...
      $columnstodisplay['importseo'] = in_array('view', $cols) ? 'icon' : null;
...
case 'importseo':
                        ...
                        $db = $gCms->GetDB();
                        $rec[$column] = '';
                        if ($this->_module->CheckPermission('Import SEO') && !$this->_is_locked($page_id) && !$this->_is_default_locked()) {
                            $gSEO = $db->GetRow('SELECT item_id,active FROM ' . cms_db_prefix() . 'module_liseseo_item WHERE title="' . $content->Alias() . '"');
                            $seo_id = ($gSEO) ? $gSEO["item_id"] : "";

                            if ($seo_id) {
                                $seo_active = $gSEO["active"];
                                if ($seo_active == 1) {
                                    $rec[$column] = "Done|" . $seo_id;
                                } else {
                                    $rec[$column] = "Exist|" . $seo_id;
                                }
                            } else {
                                $rec[$column] = "Empty";
                            }
