...
try {
    ...
    $mle_module = false;
    if( is_object($modobj) && $content_id >= 1 && $langs_counter !=0) {
        $mle_module = true;
    }
    #End MLE
        ...
        if( $currenttab == $content_obj::TAB_MAIN ) {
            ...
            if (isset($hls)) {
                ...
                foreach ($_GET as $key => $val)
                    if ('hl' != $key)
                        $thisURL .= $key . '=' . $val . '&amp;';
                $_mle_dd = '<select name="hl" onchange="window.location.href=\'' . $thisURL . 'hl=\'+this.options[this.selectedIndex].value;" class="standard">';
                foreach ($hls as $key => $mle) {
                    $_mle_dd .= '<option value="' . $key . '"';
                    if ($hl == $key)
                        $_mle_dd .= ' selected="selected"';
                    $_mle_dd .= ">" . (isset($mle['text']) ? $mle['text'] : $key) . "</option>\n";
                }
                $_mle_dd .= "</select>";
            }
            #End MLE
