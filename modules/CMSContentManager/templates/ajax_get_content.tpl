  ...
  {if isset($content_list)}
    {function do_content_row}
      {foreach $columns as $column => $flag}
        {if !$flag}{continue}{/if}
	<td {if $column == 'importseo' && $row.importseo == 'Empty'}class="init-ajax-toggle"{/if}>
	  {if $column == 'expand'}
	    {if $row.expand == 'open'}
	  ...
	  {elseif $column == 'default'}
	    ...
	    {/if}
          {elseif $column == 'importseo'}
                {assign var=foo value="|"|explode:$row.importseo}
                {if $row.importseo == 'Empty'}
                <a href="{cms_action_url action='admin_ajax_importseo' alias=$row.alias}" accesskey="s">
                    {admin_icon icon='back.gif'}
                </a>
                {elseif $foo[0] == 'Exist'}
                <a href="{cms_action_url module='LISESEO' action='admin_edititem' item_id=$foo[1]}" accesskey="s" target="_blank">
                    {admin_icon icon='run.gif'}
                </a>
                {elseif $foo[0] == 'Done'}
                <a href="{cms_action_url module='LISESEO' action='admin_edititem' item_id=$foo[1]}" accesskey="s" target="_blank">
                    {admin_icon icon='accept.gif'}
                </a>
                {/if}
	  {elseif $column == 'move'}
	    {if isset($row.move)}
