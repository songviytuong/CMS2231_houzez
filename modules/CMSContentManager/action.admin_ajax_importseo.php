...
if ($alias) {
    ...
    if ($exists) {
        ...
        $query = "INSERT INTO " . cms_db_prefix() . "module_liseseo_item (`title`,`desc`,`owner`) VALUES ('" . $sku . "', 'Import SEO From CMSContentManager', " . get_userid() . ")";
        $db->Execute($query);
        $item_category_id = $db->Insert_ID();
        $item_category_name = $db->GetOne("SELECT category_id FROM " . cms_db_prefix() . "module_liseseo_category WHERE category_alias = 'page'");
