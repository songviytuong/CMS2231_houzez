...
class TemplateValidationEditor extends ValidationEditor
{
    private $_validator;
    private $_mod;
    ...
    {
        $this->_mod = $mod;
        $this->_validator = new TemplateValidation;
    }

    public function get_name()
    {
        return $this->_mod->Lang('name_TemplateValidation');
    }
    ...
    {
        if( ! $in instanceof TemplateValidation ) throw new \LogicException('Invalid validation passed to '.__METHOD__);
        $this->_validator = $in;
    }
    ...
    public function handle_edit_form( array $params, \CGBetterForms\Form $form, $form_guid, $guid )
    {
        ...
        $tpl = $mod->CreateSmartyTemplate('admin_edit_templatevalidation.tpl');
