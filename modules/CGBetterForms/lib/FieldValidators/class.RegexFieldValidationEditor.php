...
class RegexFieldValidationEditor extends ValidationEditor
{
    private $_validator;
    private $_mod;
    ...
    {
        $this->_mod = $mod;
        $this->_validator = new RegexFieldValidation;
    }

    public function get_name()
    {
        return $this->_mod->Lang('name_RegexFieldValidator');
    }
    ...
    {
        if( ! $in instanceof RegexFieldValidation ) throw new \LogicException('Invalid validation passed to '.__METHOD__);
        $this->_validator = $in;
    }
    ...
    public function set_field( $fieldname )
    {
        ...
        if( !$fieldname ) throw new \LogicException('The field name for a RegexFieldValidation cannot be empty');
    ...
    public function set_pattern( $pattern )
    {
        ...
        if( !$pattern ) throw new \LogicException('The pattern for a RegexFieldValidation cannot be empty');

        $this->_validator->set_pattern( $pattern );
    }
    ...
    public function handle_edit_form( array $params, \CGBetterForms\Form $form, $form_guid, $guid )
    {
        ...
        if( \cge_param::exists($params,'submit') ) {
            ...
            $this->set_pattern(\cge_param::get_string($params,'pattern'));

            // now have the disposition object, store it.
            $this->get_helper()->finish( $form, $form_guid, $this->get_validation(), $guid );
        }

        $tpl = $mod->CreateSmartyTemplate('admin_edit_regexfieldvalidation.tpl');
