...
class FloatFieldValidationEditor extends ValidationEditor
{
    private $_validator;
    private $_mod;
    ...
    {
        $this->_mod = $mod;
        $this->_validator = new FloatFieldValidation;
    }

    public function get_name()
    {
        return $this->_mod->Lang('name_FloatFieldValidator');
    }
    ...
    {
        if( ! $in instanceof FloatFieldValidation ) throw new \LogicException('Invalid validation passed to '.__METHOD__);
        $this->_validator = $in;
    }
    ...
    public function set_field( $fieldname )
    {
        ...
        if( !$fieldname ) throw new \LogicException('The field name for an FloatFieldValidation cannot be empty');
    ...
    public function handle_edit_form( array $params, \CGBetterForms\Form $form, $form_guid, $guid )
    {
        ...
        $tpl = $mod->CreateSmartyTemplate('admin_edit_floatfieldvalidation.tpl');
