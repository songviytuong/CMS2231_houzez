...
class EmailAdminWithReplyToEditor extends DispositionEditor
{
    private $_disp;
    private $_mod;
    ...
    {
        $this->_mod = $mod;
        $this->_disp = new EmailAdminWithReplyTo;
    }
    ...
    {
        return $this->_mod->Lang('name_EmailAdminWithReplyTo');
    }

    public function get_displaystring()
    {
        $fieldname = $this->_disp->get_fieldname();
        return $this->_mod->Lang('display_EmailAdminWithReplyTo',$fieldname);
    }
    ...
    {
        if( ! $disp instanceof EmailAdminWithReplyTo ) throw new \LogicException('object of type '.get_class($disp).' passed to '.__METHOD__);
        $this->_disp = $disp;
    }

    public function get_outputclass()
    {
        return 'CGBetterForms\\Dispositions\\EmailAdminWithReplyTo';
    }
    ...
    public function handle_edit_form(array $params, \CGBetterForms\Form $form, $form_guid, $disp_guid )
    {
        ...
        $tpl = $mod->CreateSmartyTemplate('admin_edit_emailadminwithreplyto_disposition.tpl');
