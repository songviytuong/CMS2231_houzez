...
if( $name == $this->GetPreference('default_marker') )
  {
    $this->SetError($this->Lang('error_cannotdelete'));
    $this->RedirectToTab($id);
  }

$query = 'DELETE FROM '.cms_db_prefix().'module_cggooglemaps2_icons WHERE id = ?';
$db->Execute($query,array((int)$params['icon']));

$this->SetMessage($this->Lang('msg_icon_removed'));
$this->RedirectToTab($id);
// EOF
?>