...
if(file_exists($current_dir))
{
	...
	if ($usefm)
	{
		if (isset($params['upload_file']) && $params['upload'])
		{
			if (isset($_FILES[$id.'newfile']))
			{
				...
				if ($_FILES[$id.'newfile']['name'] != '')
				{
					...
					{
						$filename = jmfp_utils::CleanPath($_FILES[$id.'newfile']['name'], false);
						...
						if ($resize)
						{
							...
							if (jmfp_utils::HandleFileResizing($_FILES[$id.'newfile']['tmp_name'], $filename, $width, $height, $keep_aspectratio, ($params['allow_upscaling'] && $params['force_upscaling']), 100, false))
							{
								...
								$value = jmfp_utils::CleanPath($dir.'/'.$_FILES[$id.'newfile']['name'],false);
							}
							else
							{
		...
		if (isset($params['create_dir']) && $params['create_dirs'])
		{
			...
			{
				$new_dir = jmfp_utils::CleanPath($params['newdir'],false);
		...
		{
			$filename = jmfp_utils::CleanPath(base64_decode($params['deletefilename']),false);
			if (@unlink($current_dir.$filename))
			{
				//@unlink($current_dir.'thumb_'.$filename);
				@unlink(cms_join_path($config['previews_path'] , 'JMFilePickerThumbs' , 'thumb_' . munge_string_to_url(str_replace($config['uploads_path'], '', $current_dir)) . '_' . $filename));
		...
		{
			$filename = jmfp_utils::CleanPath(base64_decode($params['deletesubdir']),false);
	...
	foreach($files as $k=>$file)
	{
		if ($file->is_dir)
		{
			...
			if (jmfp_utils::IsDirEmpty($current_dir.$file->basename) && ($usefm && $params['delete']))
			{
				...
				$files[$k]->deletelink = str_replace('&amp;','&',
					$this->CreateLink(
						...
						'delete',
						$returnid,
						jmfp_utils::GetActionIcon('delete'),
	...
	if (trim($dir, './') != '' && $params['show_subdirs'])
	{
		...
		$parent_dir->fileicon     = jmfp_utils::GetFileIcon('',true);
	...
	$smarty->assign('jmfp_thumb_width', get_site_preference('thumbnail_width',96));
	$smarty->assign('jmfp_thumb_height', get_site_preference('thumbnail_height','auto'));
	$smarty->assign('jmfp_filesize_text', $this->Lang('file_size'));
	$smarty->assign('jmfp_imagesize_text', $this->Lang('image_size'));
	$smarty->assign('jmfp_files', $files);
	
	if($this->_isAdmin || $params['show_subdirs'])
	{
		$_dir = '';
		$breadcrumbs = array();
		foreach(jmfp_utils::CleanArray(explode('/', $dir)) as $k => $v)
		{
			$_dir .= $v;
		...
		$smarty->assign('jmfp_breadcrumbs', $breadcrumbs);
		$smarty->assign('jmfp_breadcrumbs', $breadcrumbs);
		...
		$smarty->assign('jmfp_startlink',$start_link);
		$smarty->assign('jmfp_starturl',$start_url);
		$smarty->assign('jmfp_startdir_text', $this->lang('startdir'));
		$smarty->assign('jmfp_startdir', $params['start_dir']);
	}
	
	// if filemanagement allowed
	if ($usefm && ($params['upload'] || $params['create_dirs']))
	{
		$smarty->assign('jmfp_fileoperations_text', $this->Lang('fileoperations'));
		...
		{
			$fileoperations_display = get_preference($userid,'JMFP_fileoperations_display',1);
		}
		$smarty->assign('jmfp_fileoperations_display', $fileoperations_display);
		$smarty->assign('jmfp_prefurl',str_replace('&amp;','&',
			$this->CreateLink($id,
				'savePrefs',
				$returnid,
				...
				'',
				true)));
		$smarty->assign('jmfp_preflink',str_replace('&amp;','&',
			$this->CreateLink($id,
				'savePrefs',
				$returnid,
				$this->lang('toggle_fileoperations'),
				array('toggle'=>'fileoperations', 'display'=>''))));
		$smarty->assign('jmfp_toggle_fileoperations_text', $this->lang('toggle_fileoperations'));
		
		$smarty->assign('jmfp_formstart', $this->CreateFormStart($id, 'fileBrowser',
		...
		// ToDo !!!
		$targetid = 'JMFP_content';
		$close_browser = false;
		if($params['mode'] == 'dropdown')
		{
			$close_browser = true;
			$targetid = $_id.$name.'_JMFP_dropdown_wrapper';
		}
		// ---
		...
		{
			$smarty->assign('jmfp_createdir_text', $this->Lang('create_dir'));
			$smarty->assign('jmfp_createdir_input', $this->CreateInputText($id, 'newdir', '', 20, 255));
			$smarty->assign('jmfp_createdir_submit', $this->CreateInputSubmit($id, 'create_dir', $this->Lang('create')));
		}
		...
		else
		{
			...
			$result .= jmfp_utils::CreateFileDropdown($_id,$name,
				$current_dir,
				$value,
...
if($this->_isAdmin || $params['show_subdirs'])
{
	$smarty->assign('jmfp_currentdir_text', $this->Lang('currentdir'));
	$smarty->assign('jmfp_currentdir', $dir);
}

$smarty->assign('jmfp_id', $_id);
$smarty->assign('jmfp_mode', $params['mode']);
$smarty->assign('jmfp_delete', $usefm && $params['delete']);
$smarty->assign('jmfp_targetid', $_id.$name);
$smarty->assign('jmfp_inputid', $_id.$name);
$smarty->assign('jmfp_cssid', $_id.munge_string_to_url($name));
$smarty->assign('jmfp_is_admin', $this->_isAdmin);
$smarty->assign('jmfp_media_type', $params['media_type']);
$smarty->assign('jmfp_show_subdirs', $params['show_subdirs']);
$smarty->assign('jmfp_upload', $params['upload'] && $usefm);
$smarty->assign('jmfp_allow_scaling', $params['allow_scaling']);
$smarty->assign('jmfp_create_dirs', ($usefm && $params['show_subdirs'] && $params['create_dirs'] && ($params['mode'] != 'dropdown')));
$smarty->assign('jmfp_success_text', $this->Lang('success'));
$smarty->assign('jmfp_error_text', $this->Lang('error'));
$smarty->assign('jmfp_deleteicon', jmfp_utils::GetActionIcon('delete'));
$smarty->assign('jmfp_confirm_delete_dir', $this->lang('confirm_delete_dir'));
$smarty->assign('jmfp_confirm_delete_file', $this->lang('confirm_delete_file'));
...
echo $this->ProcessTemplate('filebrowser.tpl');
