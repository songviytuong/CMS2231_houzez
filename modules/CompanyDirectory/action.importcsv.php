...
$can_lookup = cd_utils::can_geolocate();
$smarty->assign('can_lookup',$can_lookup);

$opts = array();
$opts['error'] = $this->Lang('error_duplicate');
$opts['insert'] = $this->Lang('import_insert');
$opts['ignore'] = $this->Lang('ignore');
$opts['update'] = $this->Lang('overwrite');
if( $this->GetPreference('allow_duplicate_company_names') ) $opts['insert'] = $this->Lang('insert_duplicate');
$smarty->assign('duplicate_opts',$opts);

$smarty->assign('url_optional',!$this->GetPreference('url_required',0));
