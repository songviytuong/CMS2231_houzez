...
// okay, find the record id and make sure it matches the owner.
$query = 'SELECT id FROM '.cms_db_prefix().'module_compdir_companies WHERE owner_id = ? and id = ?';
$tmp = $db->GetOne($query,array($feu_uid,$companyid));
if( !$tmp ) return; // record not found.

$res = cd_company::delete($companyid);
