...
// second pass... calculate iorder values for all items.
...
$query = 'UPDATE '.cms_db_prefix().'module_compdir_hier SET parent_id = ?, iorder = ? WHERE id = ?';
foreach( $hier as $rid => $row ) {
  $dbr = $db->Execute($query,array($row['parent_id'],$row['iorder'],$rid));
}
