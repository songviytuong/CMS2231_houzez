...
while ($dbresult && $row = $dbresult->FetchRow()) {
  ...
  if ($onerow->item_order > 1) {
	$onerow->uplink = $this->CreateLink($id, 'movefielddef', $returnid, $theme->DisplayImage('icons/system/arrow-u.gif', $this->Lang('up'),'','','systemicon'), array('fdid'=>$row['id'], 'dir'=>'up'));
  }
  else {
	$onerow->uplink = '';
  }
  if ($max > $onerow->item_order) {
	$onerow->downlink = $this->CreateLink($id, 'movefielddef', $returnid, $theme->DisplayImage('icons/system/arrow-d.gif', $this->Lang('down'),'','','systemicon'), array('fdid'=>$row['id'], 'dir'=>'down'));
  }
  else {
	$onerow->downlink = '';
  }
